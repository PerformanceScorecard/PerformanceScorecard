...
var pagespeed_bg = {
  ...
  // panel page and the background page. This must match Extension.java.
  ...
  // A sub-object to hold GWT functions.
  gwt: {
    saveOptimizedContents: null
  },
  ...
  displayErrorAndEndCurrentRun: function(msg, e, client) {
    ...
    if (client) {
      pagespeed_bg.setErrorText(client, message);
    } else {
      pagespeed_bg.cancelAllActiveClients(message);
    }
  },
  ...
  cancelAllActiveClients: function(msg) {
    ...
    for (var i = 0, len = clients.length; i < len; ++i) {
      ...
      pagespeed_bg.setErrorText(client, msg);
    }
  },
  ...
  messageHandler: function(port, request) {
    var tab_id = port.name;

    //var logMessage = "Message handler invoked for " + port + " request " + request.type;
    ...
    } else if (request.type === pagespeed_bg.RUN_PAGESPEED_TYPE) {
      var resource_filter = localStorage.getItem('analyzeFilter');
      var locale = localStorage.getItem('localeId');
      if (!locale) {
        locale = chrome.i18n.getMessage('@@ui_locale');
      }
      var save_optimized_content = !localStorage.getItem('noOptimizedContent');
      var client = {
        tab_id: tab_id,
        request_id: request.id, // Id used to identify message at client side.
        har: request.input.har,
        document: request.input.dom,
        timeline: request.input.timeline,
        mobile: request.input.mobile,
        resource_filter: resource_filter ? resource_filter : 'all',
        locale: locale ? locale : 'en_US',
        save_optimized_content: save_optimized_content,
        port: port
      };
    ...
    } else if (request.type === pagespeed_bg.CANCEL_RUN_TYPE) {
      var tab_id = port.name;
      delete pagespeed_bg.activeClients[tab_id];
    } else {
  ...
  fetchPartialResources_: function(client) {
    ...
    for (var i = 0, len = har.entries.length; i < len; ++i) {
      ...
      // Devtools may mess up the encodings for bad requests of images.
      // https://code.google.com/p/chromium/issues/detail?id=309710
      if (entry.response.status >= 400) {
        // When the server generates a 4xx or 5xx response, DevTools may tag the
        // response as being base64-encoded even though it is not. Here, we
        // assume that all 4xx and 5xx responses are not base64-encoded. This
        // may be incorrect in some cases. We should remove this workaround once
        // the earlier linked DevTools bug has been fixed.
        if (content.encoding === 'base64') {
          content.encoding = '';
        }
        // We do not have to fetch the content for a bad request.
        continue;
      }
  ...
  loadNaclModule: function(callback) {
    ...
    pagespeed_module.addEventListener('progress',
        pagespeed_bg.onNaclLoadProgress, true);
  ...
  runPageSpeedImpl: function(client) {
    ...
    console.log("This is the message passed to the PageSpeed module");
    console.log(msg);
  ...
  onNaclLoadProgress: function(event) {
      var loadPercent = 0.0;
  ...
  onNaclResponse: function(responseMsg) {
    ...
    try {
      ...
      console.log("These are the PageSpeed Results");
      console.log(result);

    // Save optimized contents.
    if (pagespeed_bg.gwt.saveOptimizedContents == null) {
       pagespeed_bg.displayErrorAndEndCurrentRun(
           'saveOptimizedContents is null', null, client);
    } else if (typeof pagespeed_bg.gwt.saveOptimizedContents !== 'function') {
       var type = typeof pagespeed_bg.gwt.saveOptimizedContents;
       pagespeed_bg.displayErrorAndEndCurrentRun(
           'Save is not a function: ' + type, null, client);
    } else {
      if (!localStorage.getItem('showScore')) {
        if (result && result.results && 'score' in result.results) {
          // Set the score to -1, so that the score will not be shown in the
          // overview summary.
          result.results.score = -1;
        }
      }
      pagespeed_bg.gwt.saveOptimizedContents(result, function() {
        // Error will be checked in the client.
        pagespeed_bg.postMessage(
          client, pagespeed_bg.RUN_PAGESPEED_COMPLETE_TYPE, result);

      });
    }
  },

  setErrorText: function(client, msg) {
    pagespeed_bg.postMessage(
        client, pagespeed_bg.RUN_PAGESPEED_COMPLETE_TYPE, {error: msg});
  },

  postMessage: function(client, type, result) {
    if (pagespeed_bg.isClientStillActive(client)) {
